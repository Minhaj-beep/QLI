...
import { View, Text, HStack, VStack, Icon, Image, ScrollView, Button, Modal, IconButton, Heading, FormControl, Input, Select, Center } from "native-base"
...
const CourseDiscount = ({navigation}) => {
    ...
    const renderButton = (data) => {
        if(data.requestedstatus === 'CREATED'){
            return <Text bg="primary.50" onPress={()=>handleReview(data._id)} style={{fontSize:11,color: '#FFFFFF', padding:5, borderRadius:3}}>Submit for review</Text>
        } else if (data.requestedstatus === 'REQUESTED') {
            return (
                <Text bg="primary.50" onPress={()=>{
                    setRChat(true)
                    setCourseCode(data._id)
                }} style={{fontSize:11,color: '#FFFFFF', padding:5, borderRadius:3}}>Raise ticket</Text>
            )
        } else {
            return (
                <HStack space={2}>
                    ...
                    <Text bg="primary.50" onPress={()=>{handleEnableDiscount(data)}} style={{fontSize:11,color: '#FFFFFF', padding:5, borderRadius:3}}>Enable Discount</Text>
                </HStack>
            )
        }
    }
                                        ...
                                        <HStack justifyContent={'space-between'} paddingBottom={3} alignItems={'center'}>
                                            <VStack style={styles.CardContent}>
                                                <HStack justifyContent='space-between' alignItems='center' space={2}>
                                                    ...
                                                    { data.requestedstatus === 'APPROVED' ? <Text bg='#29d363' style={styles.statusBadge}>Active</Text> : null }
                                                </HStack>
